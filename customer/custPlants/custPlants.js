...
async function displayModal(data){
    ...
    html += `</div><div class="modal-body"><img class="modalImg" src="${photoUrl}" alt="Picture" width="300" height="200"/><div style="font-weight: 650">Location: ${data[0].location}</div><div style="font-weight: 650">Water Needs: ${data[0].numTimesWater}</div><div style="font-weight: 650">Sun Needs: ${data[0].sunNeeds}</div><div id="plantinfo" style="font-weight: 650">Plant Information: ${data[0].information}</div><div style="font-weight: 650">Fun Fact: ${data[0].funFact}</div>`;
    html += `<div style="font-weight: 650">Price: ${data[0].price}</div></div><div class="modal-footer"><button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Close</button>`;
    html += `</div></div></div>`;
